...
| `auth_rate_limit_per_minute` | `10` | (Phase 7) Per-IP rate limit on `POST /login` + `v3/user.authenticate`.  `0` disables rate limiting. |
...
<dbpath>/
  ...
  users/
    users.duckdb           # cluster-replicated user store (Phase 7, see § 13)
```
...
## 13. Replicated user store + authentication

bdsweb gains username/password authentication backed by a 4th fully-
replicated cluster store, alongside docs / signals / scripts.  Every
node holds every user; password hashes use argon2id; sessions are
stateless HMAC-signed tokens; admin operations replicate cluster-wide.
The same `cluster.shared_secret` already used for `v3/cluster.*` HMAC
is reused to sign session tokens.

The wire-level mechanics live in
[`CLUSTER_DETAILS.md` § 11](CLUSTER_DETAILS.md); the operator-facing
view is below.
...
cluster: {
  enabled:                    true
  shared_secret:              "..."     // also signs session tokens
  bind_url:                   "..."

  // Phase 7 — authentication
  session_ttl:                "8h"      // cookie + token lifetime
  auth_rate_limit_per_minute: 10        // 0 disables rate limiting

  // "users" is already in the default; included for visibility
  full_replication_stores:    ["docs", "signals", "scripts", "users"]
}
```

| Key | Default | Description |
|---|---|---|
| `session_ttl` | `"8h"` | Validity of the `bds_session` cookie issued by `v3/user.authenticate`. |
| `auth_rate_limit_per_minute` | `10` | Per-IP rate limit for `POST /login` + `v3/user.authenticate` (Phase 4 will mount tower-governor against this).  Set to `0` to disable. |

### On-disk layout

A 4th store joins the network/ tree:

```
<dbpath>/
  users/
    users.duckdb           # cluster-replicated user records
  network/
    ...                    # gossip + hints + tombstones + scheduler_log (unchanged)
```
...
CREATE TABLE users (
  ...
  credential_hash TEXT NOT NULL,           -- argon2id PHC string for Password
                                           -- (provider user-id for OAuth, etc.)
...
Identical recipe to the other fully-replicated stores
([§ 11](#11-fully-replicated-stores-phase-4)):

- `v3/user.add` / `v3/user.modify` / `v3/user.delete` write locally
  first, then fan out the matching `v2/user.*` to every Alive peer.
  Failed peers enqueue hints.  Deletes write a tombstone in the
  shared `tombstones.duckdb` so AE doesn't resurrect from a stale
  peer.
- Anti-entropy now sweeps `"users"` alongside `"docs"`, `"signals"`,
  `"scripts"`.  `v2/user.list_ids` advertises live + tombstoned ids;
  `v2/user.get_by_id` returns the row verbatim (including the
  credential_hash) so AE can copy it across without re-hashing.
- `v3/user.authenticate` is the public login path and is NOT
  replicated — it runs against the local row, falling back to a
  `v2/user.get_by_username` fan-out only when the local node hasn't
  replicated the user yet (i.e. the AE window after a fresh
  user.add on a peer).  Successful authentication returns a session
  token (see below).

### Argon2id hashing

Local password verification uses argon2id with the OWASP-recommended
2024 defaults (`m=19 MiB, t=2, p=1, 32-byte output`).  Salt is
per-row, randomly generated by `OsRng`.  The credential_hash column
stores the PHC string verbatim (`$argon2id$v=19$m=19456,t=2,p=1$<salt>$<hash>`)
so verifiers can read out the parameters used to hash and stay
forward-compatible if defaults change.

### OAuth / LDAP hook

`AuthMethod` is an enum (`Password`, `OAuth { provider }`,
`Ldap { server }`, `Custom { name }`) carried on every user row.
At login time the per-row method drives `VerifierRegistry::for_method`
to pick the right `CredentialVerifier` impl.  Phase 7 ships only the
password verifier; adding OAuth or LDAP later means writing one new
verifier impl and `register()`ing it at startup — no schema change.

Unknown `auth_method` wire forms decode as `Custom` rather than
`Password`, so a typo or a future method label can never silently
become a password-hash check against an OAuth row.

### Session tokens

A successful `v3/user.authenticate` returns

```text
<user_id>.<expires_at_unix_secs>.<hex_hmac_sha256>
```

where the HMAC covers `<user_id>.<expires_at>` keyed by the cluster
shared secret.  Every node has the same key, so the token verifies
on any node — no central session store.  The bdsweb middleware sets
the token in an `HttpOnly`, `SameSite=Lax` `bds_session` cookie.

Single algorithm is hard-coded — there is no `alg=none` confusion
the way generic JWTs allow.

Expiration is enforced at verify time (`now >= expires_at` → reject).
The TTL is set from `session_ttl_secs` at issue time; there is no
revocation list in v1 (a long TTL + cookie deletion are the only
controls — set the TTL appropriately for your threat model).

### First-user bootstrap

The very first `v3/user.add` against an empty user store is admitted
**unsigned** so a fresh deployment can mint its first admin without
ambient secret distribution.  Once `users.is_empty()` returns false,
the bypass closes and every subsequent admin call requires HMAC.

bdsweb honours the same bootstrap window: while the store is empty
(probed against `v3/user.list` with a 30 s cache) the auth middleware
lets every request through so the operator can hit `/admin/users` to
create the first user via the web UI.  After the first user lands,
the middleware closes the window and the cluster-wide login wall
takes effect.

### Open-access mode

When bdsweb is started without `--config` (or against a node where
`cluster.enabled = false`), no shared secret is available and bdsweb
runs in **open-access mode**: the auth middleware is a no-op and the
login page renders a yellow banner explaining why no challenge is
