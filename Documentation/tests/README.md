...
| [markov_test](markov_test.md) | `src/analysis/markov.rs` (in-module `#[cfg(test)]`) | Semi-Markov projection: empty / bad-duration / zero-duration shape, seed determinism (catches non-deterministic `HashMap` iteration in `sample_next` / `sample_gap` / `aggregate`), duration-window invariant, drain3 / normalize / identity bucketing modes, single-state self-loop, timed empirical-gap learning takes precedence over the fallback rate, `min_consensus` filter monotonicity, `max_events` cap, time-ordered output |
